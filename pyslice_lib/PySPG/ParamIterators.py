...
class ItOperator(SPGIterator):
     ...
     def set_command(self,command,separator=" "):
        ...
        try:
          ...
          print "Line: '%s' "%command,
          print "incorrect number of parameters (found %i) for iterator '%s' over '%s'"%(len(str_rest)-1,self.it_type,self.varname)
          sys.exit()
        #######################################
        ...
        try:
          ...
          print "Line: '%s' "%command,
          print "Variable '%s': Error!"%self.varname, " ",xmin,self.it_type,xstep," do not seem to tend to ",xmax
          sys.exit()
        #
        #######################################
...
class ItRepetition(SPGIterator):
	...
	self.data    = range(eval (command)  )
        ...
        print "Line: '%s'"%command," iterator '#' could not eval()"
        sys.exit()
     def is_variable(self):
        return False
